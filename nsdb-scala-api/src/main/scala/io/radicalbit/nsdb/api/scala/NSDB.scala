...
import io.radicalbit.nsdb.api.scala.Metric.{Dimension, Field}
...
object NSDB {
  ...
  lazy implicit val system = ActorSystem("nsdb-scala-api", ConfigFactory.load("scala-api"))

  def connect(host: String = host, port: Int = port) = new NSDB(host = host, port = port)
}
...
class NSDB(host: String, port: Int) {
  ...
  def metric(metric: String) = new Metric(host = host, port = port, name = metric)
...
class Metric(host: String,
             ...
             ts: Option[Long] = None)(implicit system: ActorSystem) {

  private val client = new Client(host = host, port = port)

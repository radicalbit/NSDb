...
class ReadCoordinatorGlobalAggregatedStatementsSpec extends AbstractReadCoordinatorSpec {
  ...
  "ReadCoordinator" when {
    ...
    "receive a select containing a global aggregation but empty Result set" should {
      ...
      "execute it successfully with a max()" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
              ...
              distinct = false,
              fields = ListFields(
                List(Field("*", Some(MaxAggregation("value"))))
              ),
              Some(Condition(EqualityExpression("surname", AbsoluteComparisonValue(NSDbStringType("yorke")))))
            )
          )
      ...
      "execute it successfully with a sum()" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
              ...
              distinct = false,
              fields = ListFields(
                List(Field("*", Some(SumAggregation("value"))))
              ),
              Some(Condition(EqualityExpression("surname", AbsoluteComparisonValue(NSDbStringType("yorke")))))
            )
          )
        ...
        expected.values shouldBe Seq(Bit(0, 0L, Map.empty, Map("sum(*)" -> NSDbLongType(0L))))
      }
      ...
      "execute it successfully with mixed count(), max() aggregations" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
              ...
              distinct = false,
              fields = ListFields(
                List(Field("*", Some(MaxAggregation("value"))), Field("*", Some(CountAggregation("value"))))
              ),
              condition =
                Some(Condition(EqualityExpression("surname", AbsoluteComparisonValue(NSDbStringType("yorke")))))
            )
      ...
      "execute it successfully with mixed count(), sum() aggregations" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
              ...
              distinct = false,
              fields = ListFields(
                List(Field("*", Some(SumAggregation("value"))), Field("*", Some(CountAggregation("value"))))
              ),
              condition =
                Some(Condition(EqualityExpression("surname", AbsoluteComparisonValue(NSDbStringType("yorke")))))
            )
        ...
        expected.values shouldBe Seq(Bit(0, 0L, Map.empty, Map("count(*)" -> 0L, "sum(*)" -> 0L)))
      }

      "execute it successfully with mixed max aggregation and plain fields" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
      ...
      "execute it successfully with mixed sum aggregation and plain fields" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
    ...
    "receive a select containing only one single global aggregations" should {
      ...
      "execute it successfully with a max aggregation" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
        ...
        expected.values shouldBe Seq(
          Bit(0, 0.0, Map.empty, Map("max(*)" -> NSDbDoubleType(5.0)))
        )
      }

      "execute it successfully with a sum aggregation" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
        ...
        expected.values shouldBe Seq(
          Bit(0, 0.0, Map.empty, Map("sum(*)" -> NSDbDoubleType(26L)))
        )
      }
    }
    ...
    "receive a select containing mixed global aggregations and plain fields" should {
      ...
      "execute it successfully with mixed count, max and plain fields" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
        ...
        expected.values.sortBy(_.timestamp) shouldBe Seq(
          Bit(1L, 1L, Map.empty, Map("name"  -> "John", "max(*)"    -> 6L, "count(*)" -> 6L)),
          Bit(2L, 2L, Map.empty, Map("name"  -> "John", "max(*)"    -> 6L, "count(*)" -> 6L)),
          Bit(4L, 3L, Map.empty, Map("name"  -> "J", "max(*)"       -> 6L, "count(*)" -> 6L)),
          Bit(6L, 4L, Map.empty, Map("name"  -> "Bill", "max(*)"    -> 6L, "count(*)" -> 6L)),
          Bit(8L, 5L, Map.empty, Map("name"  -> "Frank", "max(*)"   -> 6L, "count(*)" -> 6L)),
          Bit(10L, 6L, Map.empty, Map("name" -> "Frankie", "max(*)" -> 6L, "count(*)" -> 6L))
        )
      }

      "execute it successfully with mixed count, max and a condition" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
        ...
        expected.values.sortBy(_.timestamp) shouldBe Seq(
          Bit(0L, 0.0, Map.empty, Map("max(*)" -> 5.0, "count(*)" -> 4L))
        )
      }

      "execute it successfully with mixed count, sum and plain fields" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
        ...
        expected.values.sortBy(_.timestamp) shouldBe Seq(
          Bit(1L, 1L, Map.empty, Map("name"  -> "John", "sum(*)"    -> 21L, "count(*)" -> 6L)),
          Bit(2L, 2L, Map.empty, Map("name"  -> "John", "sum(*)"    -> 21L, "count(*)" -> 6L)),
          Bit(4L, 3L, Map.empty, Map("name"  -> "J", "sum(*)"       -> 21L, "count(*)" -> 6L)),
          Bit(6L, 4L, Map.empty, Map("name"  -> "Bill", "sum(*)"    -> 21L, "count(*)" -> 6L)),
          Bit(8L, 5L, Map.empty, Map("name"  -> "Frank", "sum(*)"   -> 21L, "count(*)" -> 6L)),
          Bit(10L, 6L, Map.empty, Map("name" -> "Frankie", "sum(*)" -> 21L, "count(*)" -> 6L))
        )
      }

      "execute it successfully with mixed count, sum and a condition" in {
        probe.send(
          readCoordinatorActor,
          ExecuteStatement(
            SelectSQLStatement(
        ...
        expected.values.sortBy(_.timestamp) shouldBe Seq(
          Bit(0L, 0.0, Map.empty, Map("sum(*)" -> 16.0, "count(*)" -> 4L))
        )
      }
          ...
          ExecuteStatement(
            SelectSQLStatement(
              ...
              fields = ListFields(
                List(Field("*", Some(CountAggregation("*"))),
                     Field("surname", None),
                     Field("value", Some(FirstAggregation("*"))))),
              limit = Some(LimitOperator(4))
            )
          )
        )

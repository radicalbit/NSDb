...
class WriteCoordinatorSpec
  ...
  "WriteCoordinator" should "write records" in {
    val record1 = Record(System.currentTimeMillis, Map("content" -> s"content"), 1)
    val record2 = Record(System.currentTimeMillis, Map("content" -> s"content", "content2" -> s"content2"), 2)
    val incompatibleRecord =
      Record(System.currentTimeMillis, Map("content" -> 1, "content2" -> s"content2"), 3)
  ...
  "WriteCoordinator" should "write records and publish event to its subscriber" in {
    val testRecordSatisfy = Record(100, Map("name" -> "john"), 1)
  ...
  "WriteCoordinator" should "delete entries" in {
    ...
    val records: Seq[Record] = Seq(
      Record(2, Map("name"  -> "John", "surname"  -> "Doe", "creationDate" -> System.currentTimeMillis()), 1),
      Record(4, Map("name"  -> "John", "surname"  -> "Doe", "creationDate" -> System.currentTimeMillis()), 1),
      Record(6, Map("name"  -> "Bill", "surname"  -> "Doe", "creationDate" -> System.currentTimeMillis()), 1),
      Record(8, Map("name"  -> "Frank", "surname" -> "Doe", "creationDate" -> System.currentTimeMillis()), 1),
      Record(10, Map("name" -> "Frank", "surname" -> "Doe", "creationDate" -> System.currentTimeMillis()), 1)
    )

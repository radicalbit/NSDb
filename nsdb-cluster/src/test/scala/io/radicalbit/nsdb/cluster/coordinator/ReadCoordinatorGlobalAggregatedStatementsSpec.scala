...
import io.radicalbit.nsdb.common.{NSDbDoubleType, NSDbLongType, NSDbNumericType, NSDbStringType}
  ...
  "ReadCoordinator" when {
    ...
    "receive a select containing a global aggregation but empty Result set" should {
      ...
      "execute it successfully with a min()" in {
        ...
        expected.values shouldBe Seq(Bit(0, 0L, Map.empty, Map("min(*)" -> NSDbLongType(Long.MaxValue))))
      }
    }

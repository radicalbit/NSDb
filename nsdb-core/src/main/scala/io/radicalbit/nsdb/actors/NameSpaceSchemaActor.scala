...
class NameSpaceSchemaActor(val basePath: String, namespace: String)
    extends Actor
    with SchemaSupport
    with ActorLogging {
...
object NameSpaceSchemaActor {
  ...
  def props(basePath: String, namespace: String): Props = Props(new NameSpaceSchemaActor(basePath, namespace))
